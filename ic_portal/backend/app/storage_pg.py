"""Postgres-backed storage for the Six Peak IC Weekly portal.

Uses tables prefixed with `ic_` so the IC portal can share a Neon database
with the other portals without colliding on table names.
...
SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS ic_doc (
...
CREATE INDEX IF NOT EXISTS ic_meetings_date_desc_idx ON ic_meetings (date DESC, saved_at DESC);
...
ALTER TABLE ic_meetings ADD COLUMN IF NOT EXISTS followup_sent_at TIMESTAMPTZ NULL;
ALTER TABLE ic_meetings ADD COLUMN IF NOT EXISTS followup_log JSONB NULL;
"""
...
class PostgresStorage:
    ...
    def load_doc(self) -> dict[str, Any]:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT data FROM ic_doc WHERE id = 1")
                row = cur.fetchone()
        if row is None:
    ...
    def save_doc(self, data: dict[str, Any]) -> None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO ic_doc (id, data, updated_at)
                    VALUES (1, %s, now())
    ...
    def save_meeting(self, meeting: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO ic_meetings (id, date, data)
                    VALUES (%s, %s, %s)
                    ON CONFLICT (id) DO UPDATE
    ...
    def list_meetings(self, limit: int | None = None) -> list[dict[str, Any]]:
        sql = "SELECT data FROM ic_meetings ORDER BY date DESC, saved_at DESC"
        params: tuple[Any, ...] = ()
    ...
    def get_meeting(self, meeting_id: str) -> dict[str, Any] | None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT data FROM ic_meetings WHERE id = %s", (meeting_id,))
                row = cur.fetchone()
    ...
    def record_followup_log(self, meeting_id: str, log: dict[str, Any]) -> None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE ic_meetings SET followup_log = %s WHERE id = %s",
                    (Json(log), meeting_id),
                )
            conn.commit()
