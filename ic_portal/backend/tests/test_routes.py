"""Route-level smoke tests for the Six Peak IC Weekly portal."""
...
def test_portal_renders_empty(client):
    ...
    assert "Six Peak IC Weekly" in body
...
def test_portal_renders_with_meeting_and_goals(client, storage: Storage):
    ...
    storage.save_meeting({
        "id": "m1", "date": "2026-04-22", "title": "Six Peak IC Weekly Apr 22",
        "summary": "First. Second.", "action_items": [], "files": [],
    })
    body = client.get("/").data.decode()
    assert "Six Peak IC Weekly Apr 22" in body
...
def test_ingest_webhook_accepts_ic(client, storage: Storage):
    payload = {"meeting": {
        "id": "ic1", "title": "Six Peak IC Weekly Meeting",
    ...
    m = storage.get_meeting("ic1")
    assert m is not None
    # Action items got IDs assigned at ingest
...
def test_ingest_maps_readai_text_to_task(client, storage: Storage):
    ...
    payload = {"meeting": {
        "id": "rai1", "title": "Six Peak IC Weekly",
...
def test_portal_renders_text_only_action_items(client, storage: Storage):
    storage.save_meeting({
        "id": "legacy", "date": "2026-04-28", "title": "Six Peak IC Weekly",
        "summary": "",
...
def test_ingest_filter_rejects_unrelated_ic_mentions(client, storage: Storage):
    """Generic 'IC' mentions in unrelated titles should be ignored."""
    payload = {"meeting": {
        "id": "off-1",
        "title": "Topic IC discussion sidebar",
        "date": "2026-04-29",
    }}
    r = client.post("/api/ingest/readai", json=payload, headers={"X-API-Key": "test-key"})
    assert r.status_code == 200
    assert r.get_json()["status"] == "ignored"
    assert storage.get_meeting("off-1") is None


def test_ingest_filter_accepts_six_peak_ic(client, storage: Storage):
    payload = {"meeting": {
        "id": "spic-1",
        "title": "Six Peak IC weekly check-in",
        "start_time": "2026-04-28T15:00:00Z",
        "summary": "s",
        "action_items": [],
    }}
...
def test_ingest_filter_accepts_investment_committee(client, storage: Storage):
    payload = {"meeting": {
        "id": "icc-1",
        "title": "Investment Committee — Q2 Review",
        "start_time": "2026-04-28T15:00:00Z",
        "summary": "s",
        "action_items": [],
    }}
...
def test_ingest_extracts_owner(client, storage: Storage):
    payload = {"meeting": {
        "id": "owner1", "title": "Six Peak IC Weekly Meeting",
        "start_time": "2026-04-28T15:00:00Z",
        "summary": "s",
        "action_items": [
...
def test_ingest_purges_completed_todos(client, storage: Storage):
    ...
    payload = {"meeting": {
        "id": "ic2", "title": "Six Peak IC Weekly", "start_time": "2026-04-22T15:00:00Z",
