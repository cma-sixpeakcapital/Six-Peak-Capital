"""Render the LV Exec mid-cycle reminder email — pure functions, no I/O.
...
the next bi-weekly LV Exec call, not a personal note. The header makes
that explicit so recipients don't reply expecting Chris on the other end.
"""
...
def estimated_next_call(meeting_date: str | date_cls) -> str:
    """For a bi-weekly cadence, estimate the next call as +14 days."""
...
def _open_goals(goals: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Filter goals down to those still incomplete. LV Exec goals are a
    flat list (same shape as Finance) — no per-person grouping."""
...
def render_html(meeting: dict[str, Any], open_todos: list[dict[str, Any]],
    ...
    parts.append(
        ...
        f"call on {escape(next_call)}. Below are the open to-dos and goals "
