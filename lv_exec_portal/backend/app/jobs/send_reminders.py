"""LV Exec mid-cycle reminder email job — runs hourly via GitHub Actions cron.

Triggered by ``POST /api/jobs/send_reminders`` (auth: PORTAL_API_KEY header).
For each LV Exec meeting where:

  - ``reminder_sent_at`` is NULL
  - 7d ≤ (today - meeting.date) ≤ 14d
  - summary is non-empty
  - there's at least one open to-do OR open goal (silent-skip when both empty)

Reuses helpers from ``send_followups.py`` (OAuth creds, Gmail/Calendar
service builders, MIME helpers, invitee filter). Only the SQL filter, the
template, and the column names differ.
"""
...
from .reminder_template import _open_goals, render_email
