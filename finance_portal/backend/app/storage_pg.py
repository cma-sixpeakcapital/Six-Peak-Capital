"""Postgres-backed storage for the Six Peak Monthly Finance portal.

Uses tables prefixed with ``finance_`` so this portal can share a Neon
database with the L10 and LV Exec portals without colliding on table
names. Connection strategy: per-request psycopg.connect with 30s
connect_timeout (handles Neon free-tier cold-start).
"""
import json
...
SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS finance_doc (
...
-- Mid-cycle reminder columns (Finance-only — 14 days after the monthly call,
-- a "still open?" nudge goes out so the team has 2 weeks before the next one).
...
ALTER TABLE finance_meetings ADD COLUMN IF NOT EXISTS reminder_sent_at TIMESTAMPTZ NULL;
ALTER TABLE finance_meetings ADD COLUMN IF NOT EXISTS reminder_log JSONB NULL;
"""
...
class PostgresStorage:
    ...
    def load_doc(self) -> dict[str, Any]:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT data FROM finance_doc WHERE id = 1")
                row = cur.fetchone()
        if row is None:
    ...
    def save_doc(self, data: dict[str, Any]) -> None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO finance_doc (id, data, updated_at)
                    VALUES (1, %s, now())
    ...
    def save_meeting(self, meeting: dict[str, Any]) -> dict[str, Any]:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO finance_meetings (id, date, data)
                    VALUES (%s, %s, %s)
                    ON CONFLICT (id) DO UPDATE
    ...
    def list_meetings(self, limit: int | None = None) -> list[dict[str, Any]]:
        sql = "SELECT data FROM finance_meetings ORDER BY date DESC, saved_at DESC"
        params: tuple[Any, ...] = ()
    ...
    def get_meeting(self, meeting_id: str) -> dict[str, Any] | None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT data FROM finance_meetings WHERE id = %s", (meeting_id,))
                row = cur.fetchone()
    ...
    # Mirrors the follow-up methods above but uses reminder_sent_at / reminder_log
    # columns, and anchors on the meeting's calendar DATE (not saved_at) so
    ...
    def list_meetings_pending_reminder(
        ...
        Filters:
          ...
          - meeting date is between min_age_days and max_age_days ago
            (anchored on data->>'date' which is a stable calendar date)
          - summary is non-empty (we won't nudge people about a no-content recap)

        Open-todos check happens in Python — the to-dos live in a separate
        finance_doc table, not in the meeting row, so SQL can't join cleanly.
    ...
    def record_reminder_log(self, meeting_id: str, log: dict[str, Any]) -> None:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE finance_meetings SET reminder_log = %s WHERE id = %s",
                    (Json(log), meeting_id),
                )
            conn.commit()
